...
pub fn EncodeV4Public(comptime name: []const u8) type {
    return struct {
        ...
        pub fn encode(self: Self, r: std.Random, msg: []const u8, key: Ed25519.SecretKey, f: []const u8, i: []const u8) ![]u8 {
            ...
            // Combine message + sign for base64 encoding
...
test "V4Public fail" {
    {
        ...
        const res = e.decode(encoded3[0..], pubkey, f, i);
        try testing.expectError(error.PasetoInvalidTokenSignature, res);
    }
}

test "V4Public Decrypt fail" {
    const key = "1eb9dbbbbc047c03fd70604e0071f0987e16b28b757225c11f00415d0e20b1a2";
    ...
    const res = e.decode(encoded3[0..25], pubkey, f, i);
    try testing.expectError(error.PasetoIncorrectTokenFormat, res);

    const encoded1 = "";
